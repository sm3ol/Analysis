...
def placeholder_clean_reference_stats(latent_dim: int, device: torch.device) -> CleanReferenceStats:
    """Construct an identity clean reference before the first fit."""
    mu = torch.zeros((latent_dim,), device=device, dtype=torch.float32)
    cov = torch.eye(latent_dim, device=device, dtype=torch.float32)
    return CleanReferenceStats(mu_clean=mu, cov_clean=cov, precision_clean=cov.clone())


def mahalanobis_distance(z: torch.Tensor, stats: CleanReferenceStats) -> torch.Tensor:
