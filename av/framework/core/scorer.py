...
class BrainBScorer(nn.Module):
    ...
    def update_stats(self, stats: CleanReferenceStats) -> None:
        """Hot-swap Brain-B clean statistics after a refit."""
        self.mu_clean.copy_(stats.mu_clean.to(self.mu_clean.device))
        self.precision_clean.copy_(stats.precision_clean.to(self.precision_clean.device))
