#!/usr/bin/env python
"""Taxonomy-oriented family similarity metrics on MediMeta-C.
...
- Metrics pooled across modalities + per-modality
...
Previous ViT profile (already executed):
- Encoders: clip_vit, biomedclip_vit
- Corruptions: all available MediMeta-C corruptions
- Severities: typically run as 1..5 (often one severity per run directory)
- Recommended output root for this profile: outputs/taxonomy_stage2_sev{severity}
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="MediMeta-C taxonomy metrics")
    p.add_argument(
        "--data_root",
        type=str,
        default="data/MediMeta-C",
        help="Path to MediMeta-C root",
    )
    p.add_argument(
        "--output_dir",
        type=str,
        default="outputs/taxonomy_metrics",
        help="Output directory root",
    )
    p.add_argument(
        "--encoders",
        type=str,
        default="clip_vit,biomedclip_vit",
    ...
    p.add_argument(
        "--max_samples_per_modality",
        type=int,
        default=512,
        help="Max clean/corrupted matched samples per modality",
    )
    p.add_argument(
        "--batch_size",
        type=int,
...
def _list_modalities(data_root: Path) -> List[str]:
    modalities: List[str] = []
    for p in sorted(data_root.iterdir()):
        if p.is_dir() and not p.name.startswith("."):
            modalities.append(p.name)
    return modalities


def _discover_corruptions(modality_dir: Path, split: str) -> List[str]:
    corrs = set()
    split_dir = modality_dir / split
    for f in split_dir.glob("*_severity_*.npz"):
        name = f.name
        if "_severity_" not in name:
            continue
        corrs.add(name.split("_severity_")[0])
    return sorted(corrs)
...
def _check_matched_indices(
    ...
    if strict and not labels_match:
        raise ValueError(
            f"Matched-index check failed for {modality}/{split}/{corruption}/sev{severity}: "
            f"{mismatch} mismatched label entries"
        )
    return check


def _instantiate_encoder(name: str, device: str):
    if name not in ENCODER_REGISTRY:
        raise ValueError(f"Unknown encoder '{name}'. Available: {sorted(ENCODER_REGISTRY)}")
    cls, vision_cls = ENCODER_REGISTRY[name]
...
def _build_points_for_encoder(
    ...
    for modality in modalities:
        split_dir = data_root / modality / split
        clean_path = split_dir / "clean.npz"
        if not clean_path.exists():
            summary["groups_skipped"] += 1
            continue

        clean_imgs, clean_labels = _load_npz(clean_path)
        sel = _select_indices(len(clean_imgs), max_samples_per_modality, rng)
        ...
        for corr in corruptions:
            ...
            for sev in severities:
                corr_path = split_dir / f"{corr}_severity_{sev}.npz"
                if not corr_path.exists():
                    summary["groups_skipped"] += 1
                    continue

                corr_imgs, corr_labels = _load_npz(corr_path)
                if len(corr_imgs) < (int(sel[-1]) + 1 if len(sel) else 0):
                    raise ValueError(
                        f"{corr_path} has fewer samples ({len(corr_imgs)}) than selected max index"
                    )
                ...
                if aggregation == "per_image":
                    ...
                    for ii in keep.tolist():
                        ...
                        metas.append(
                            PointMeta(
                                point_id=f"{modality}|{corr}|s{sev}|i{int(sel[ii])}",
                                modality=modality,
                                corruption=corr,
                                severity=int(sev),
                                family=fam,
                    ...
                    metas.append(
                        PointMeta(
                            point_id=f"{modality}|{corr}|s{sev}",
                            modality=modality,
                            corruption=corr,
                            severity=int(sev),
                            family=fam,
        ...
        if modality_done:
            summary["modalities_processed"].append(modality)

    if not vectors:
        raise ValueError("No valid Delta points were built. Check modalities/corruptions/severities.")
...
def main() -> None:
    ...
    all_modalities = _list_modalities(data_root)
    if args.modalities == "all":
        modalities = all_modalities
    else:
        req = _parse_csv_list(args.modalities)
        missing = [m for m in req if m not in all_modalities]
        if missing:
    ...
    if args.corruptions == "all":
        discovered = _discover_corruptions(data_root / modalities[0], args.split)
        corruptions = discovered
    else:
    ...
    config = {
        ...
        "max_samples_per_modality": args.max_samples_per_modality,
    ...
    for enc in encoders:
        ...
        model = _instantiate_encoder(enc, device=device)
        ...
        modalities_per_point = [m.modality for m in eval_metas]

        # Export metadata rows once per encoder (shared across variants).
        ...
        # Scopes: pooled + each modality.
        scope_to_indices: Dict[str, np.ndarray] = {
            "pooled": np.arange(len(eval_metas), dtype=np.int64)
        }
        for mod in sorted(set(modalities_per_point)):
            idx = np.where(np.array(modalities_per_point) == mod)[0]
            scope_to_indices[f"modality:{mod}"] = idx
        ...
        # Free model aggressively between encoders.
        del model
